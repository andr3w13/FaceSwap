...
def random_warp(image):
    ...
    interp_mapx = cv2.resize(mapx, (int(w / 2 * (1 + 0.25)), int(h / 2 * (1 + 0.25))))[
                  int(w / 2 * 0.25 / 2):int(w / 2 * (1 + 0.25) - w / 2 * 0.25 / 2),
                  int(w / 2 * 0.25 / 2):int(w / 2 * (1 + 0.25) - w / 2 * 0.25 / 2)].astype('float32')
    interp_mapy = cv2.resize(mapy, (int(w / 2 * (1 + 0.25)), int(h / 2 * (1 + 0.25))))[
                  int(w / 2 * 0.25 / 2):int(w / 2 * (1 + 0.25) - w / 2 * 0.25 / 2),
                  int(w / 2 * 0.25 / 2):int(w / 2 * (1 + 0.25) - w / 2 * 0.25 / 2)].astype('float32')
    ...
    A = np.hstack((A, np.tile(np.eye(2), (
    src_points.shape[0], 1))))  # [x, -y, 1, 0] for x coordinate and [y, x, 0 ,1] for y coordinate
...
class FaceData(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        """
        images_src, images_dst = list(
            zip(*batch))  # convert list of tuples with pairs of images into tuples of source and destination images
...
def dssim(image1, image2, window_size=11):
    """
    Calculate the structural dis-similarity index measure.
    Args:
